	...
	return worker.Serve(ctx, *socket, worker.Initialize{
		...
		case "jobs", "retry-issue":
			if request.Mode == "retry-issue" {
				if request.Issue < 1 {
					return worker.Result{}, fmt.Errorf("retry requires an issue")
				}
			...
			jobs := map[int]*worker.JobSummary{}
			if saved != nil {
				for number, j := range saved.Jobs {
					v := &worker.JobSummary{Status: j.Status, Failure: j.Failure, ClaimPending: j.ClaimPending, Tries: j.Tries, RetryAt: j.RetryAt, URL: j.URL, Branch: j.Branch}
					if j.Result != nil {
						v.Result = &worker.JobResult{Status: j.Result.Status, Detail: j.Result.Detail}
					}
					jobs[number] = v
				}
			}
			return worker.Result{Jobs: jobs}, nil
		case "":
		default:
