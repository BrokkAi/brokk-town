...
func dashboardFixture() *dashboard {
	...
	cfg.GitHub.Repo = "BrokkAi/repo-bot"
	cfg.Branch = "main"
	cfg.Agent.Model = "review-model"
	cfg.StateDirectory = "/tmp/repo-bot-state"
	d := newDashboard(cfg)
	d.update(bot.Progress{Phase: "starting"})
	d.update(bot.Progress{Phase: "repairing", Task: "Repairing test, lint", Commit: strings.Repeat("a", 40),
		Counts: bot.ObservationCounts{Health: "red", OpenIssues: 12, OpenPulls: 3, Releases: 5, Observations: 3, Red: 1, Repairs: 1}, Items: []bot.ItemProgress{
			{ID: "a", Title: "Sep 21 10:00 · aaaaaaaa · 12 issues · 3 PRs", Status: "repaired", URL: "https://github.com/BrokkAi/repo-bot/commit/abc", Body: strings.Repeat("Published repair abc.\n", 20)},
			{ID: "b", Title: "Sep 22 10:00 · bbbbbbbb · 12 issues · 3 PRs", Status: "green"},
			{ID: "c", Title: "Wide text 日本語 👩‍💻 é failing test", Status: "red"},
		}})
	d.runs, d.sessions, d.tools = 2, 4, 23
	d.tool = "go test ./..."
	for _, text := range []string{"Reading checks", "Repairing test", "Verifying"} {
...
func executeWithRun(ctx context.Context, args []string, log *slog.Logger, run runFunc) error {
	return executeWith(ctx, args, log, run, os.Stdout)
}
...
func TestDashboardFitsPaneAndUnicode(t *testing.T) {
	...
	for _, want := range []string{"BrokkAi/repo-bot", "main", "REPAIRING", "RED", "12 issues", "3 PRs", "5 releases", "2 observations", "4 agents", "23 tools", "go test ./..."} {
...
func TestDashboardItemDetailsRemainSelected(t *testing.T) {
	...
	if !strings.Contains(text, "REPAIRED") || !strings.Contains(text, "https://github.com/") {
...
func TestDashboardSummaryKeepsLinksAndNewItems(t *testing.T) {
	...
	if !strings.Contains(summary, "branch RED") || !strings.Contains(summary, "12 issues, 3 PRs") {
		t.Fatalf("summary: %s", summary)
	}
	d.update(bot.Progress{Items: []bot.ItemProgress{{ID: "red", Title: "Failing", Status: "red", Body: "Failing checks\ntest"}}})
	if !strings.Contains(d.summary(nil), "Failing checks\ntest") {
		t.Fatal("failing observation lost on exit")
	}
}
