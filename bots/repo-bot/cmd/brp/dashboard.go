...
type dashboard struct {
	...
	streamKey                           string
	streamOpen                          bool
	runs, sessions, tools, failures     int
...
func newDashboard(cfg bot.Config) *dashboard {
	...
	return &dashboard{cfg: cfg, started: now, phaseStarted: now, lastActivity: now,
		progress: bot.Progress{Phase: "starting", Task: "Loading saved observations"}}
}
...
func (d *dashboard) update(p bot.Progress) {
	...
	if (d.detail || d.selected > 0) && len(d.progress.Items) > d.selected {
		id := d.progress.Items[len(d.progress.Items)-1-d.selected].ID
		for i, f := range p.Items {
			if f.ID == id {
				d.selected = len(p.Items) - 1 - i
				break
			}
		}
	}
	d.progress = p
	d.selected = max(0, min(d.selected, len(p.Items)-1))
}
...
func (d *dashboard) key(key string) bool {
	...
	case "enter":
		if len(d.progress.Items) > 0 && d.view != 2 {
	...
	case "j", "down", "pgdown":
		if d.view == 2 {
			...
			d.selected = max(0, min(len(d.progress.Items)-1, d.selected+1))
		}
	case "k", "up", "pgup":
		if d.view == 2 {
			d.scroll++
	...
	case "G":
		if d.view == 2 {
			...
			d.selected = max(0, len(d.progress.Items)-1)
		}
	}
	return false
}
...
func (d *dashboard) render(width, height int, now time.Time, color bool) string {
	...
	add("REPO BOT  "+d.cfg.GitHubRepo(), "1;32")
	if d.detail && len(p.Items) > 0 {
		add(fmt.Sprintf("OBSERVATION DETAIL · %d/%d", d.selected+1, len(p.Items)), "2")
		f := p.Items[len(p.Items)-1-d.selected]
	...
	c := p.Counts
	health := "no observation yet"
	if c.Health != "" {
		health = statusLabel(c.Health)
	}
	add(fmt.Sprintf("Branch: %s · %d issues · %d PRs · %d releases", health, c.OpenIssues, c.OpenPulls, c.Releases), "1")
	if height >= 18 {
		add(fmt.Sprintf("Saved: %d observations · %d red · %d repaired", c.Observations, c.Red, c.Repairs), "2")
	}
	run := fmt.Sprintf("Run: %d observations · %d agents · %d tools", d.runs, d.sessions, d.tools)
	if width >= 65 {
		run = fmt.Sprintf("Run: %d observations · %d agents · %d tools · %d errors", d.runs, d.sessions, d.tools, d.failures)
	}
	add(run, "2")
	if height >= 18 {
		add("", "")
	}
	tabs := []string{"1 Overview", "2 Observations", "3 Activity"}
	...
	for _, line := range body {
		...
		if d.view != 2 {
			if strings.HasPrefix(line, "› ") {
				style = "1;36"
			} else if strings.HasPrefix(line, "  GREEN ") || strings.HasPrefix(line, "  REPAIRED ") {
				style = "32"
			}
		}
		add(line, style)
...
func (d *dashboard) findingRows(limit int) []string {
	...
	findings := d.progress.Items
	if len(findings) == 0 {
		if d.progress.Phase == "waiting" || d.progress.Phase == "complete" {
			return []string{"No observations in saved history."}
		}
		return []string{"No observations yet."}
	}
...
func (d *dashboard) summary(err error) string {
	...
	text := fmt.Sprintf("brp %s · %s · branch %s · %d issues, %d PRs, %d releases\n", cleanText(d.cfg.GitHubRepo()), status, health, c.OpenIssues, c.OpenPulls, c.Releases)
	for _, f := range d.progress.Items {
		...
		if f.Status == "red" || f.Status == "unrepairable" || f.Status == "error" {
