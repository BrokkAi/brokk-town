    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for target in release.TARGETS:
            ...
            with tarfile.open(assets / name, "r:gz") as bundle:
                files = {member: bundle.extractfile(member).read() for member in ("brp", "README.md", "BUILD.json", *licenses.LEGAL_FILES)}
            ...
            npm_pack(package_name, {"os": [system], "cpu": [arch], "description": f"Brokk Repo Bot native binary for {system}/{arch}"},
                     {"bin/brp": files["brp"], **{name: files[name] for name in licenses.LEGAL_FILES}, "README.md": files["README.md"], "BUILD.json": files["BUILD.json"]})
        npm_pack(NPM_ROOT, {
            "description": "Brokk Repo Bot: autonomous issue fixes through Agent Client Protocol",
            "bin": {"brp": "bin/brp.cjs"}, "engines": {"node": ">=18"},
            "os": ["linux", "darwin"], "cpu": ["x64", "arm64"], "optionalDependencies": dependencies,
        }, {"bin/brp.cjs": (ROOT / "npm/brp.cjs").read_bytes(), **{name: files[name] for name in licenses.LEGAL_FILES}, "README.md": files["README.md"]})
