...
main() {
    ...
    if [ "$version" = latest ]; then
        ...
        registry=$(download "https://registry.npmjs.org/@brokkai/repo-bot/latest") || fail 'could not resolve bot version'
        version=$(printf '%s' "$registry" | python3 -c 'import json,sys; print("v"+json.load(sys.stdin)["version"])') || fail 'invalid bot version'
    fi
    case "$version" in *-repo-bot) ;; *) version=$version-repo-bot ;; esac
    printf '%s\n' "$version" | awk '
