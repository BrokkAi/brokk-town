...
def context(tag_arg=None, sha_arg=None):
    ...
    if not tag.endswith("-review-bot"):
        raise ValueError("release tag must end in -review-bot")
    ...
    if manifest_path.exists():
        ...
        entry = next(b for b in entries if b["project"] == "review-bot")
...
def publish(directory, tag, sha, check_only=False):
    ...
    if record is None:
        record = api("releases", "POST", {
            "tag_name": tag, "target_commitish": sha, "name": f"review-bot {tag}",
