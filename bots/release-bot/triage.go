...
// the fixed cadence. It runs once per observed pair of local and remote heads
// after the quiet period, ignores the minimum gap, and never bypasses the need
// for unreleased commits or the daily deadline, which due already enforces.
// A saved record without a remote head predates that key and is reassessed.
func (e *engine) triage(ctx context.Context, s *State, head, remoteHead string, total int, now time.Time) (string, error) {
	...
	e.log.Info("Triaging unreleased commits", "head", head, "remote_head", remoteHead, "unreleased_commits", total)
