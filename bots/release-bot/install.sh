...
fail() {
    printf 'brokk-release-bot installer: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    if [ "$version" = latest ]; then
        command -v python3 >/dev/null 2>&1 || fail 'python3 is required to resolve the latest version'
        registry=$(download "https://registry.npmjs.org/@brokkai/release-bot/latest") || fail 'could not resolve bot version'
        version=$(printf '%s' "$registry" | python3 -c 'import json,sys; print("v"+json.load(sys.stdin)["version"])') || fail 'invalid bot version'
    fi
    case "$version" in *-release-bot) ;; *) version=$version-release-bot ;; esac
    printf '%s\n' "$version" | awk '
    ...
    [ ! -d "$install_dir/brb" ] || fail 'installation target is a directory'
    temporary=$(mktemp -d)
    staged=
    trap 'rm -rf "$temporary"; if [ -n "$staged" ]; then rm -f "$staged"; fi' 0
    trap 'exit 1' HUP INT TERM

    asset=brokk-release-bot-$version-$os-$arch.tar.gz
    printf 'Downloading brokk-release-bot %s for %s/%s...\n' "$version" "$os" "$arch"
    ...
    mkdir -p "$install_dir"
    staged=$(mktemp "$install_dir/.brokk-release-bot.XXXXXX")
    # Extract only the binary to a fresh file; never follow archive paths or links.
    tar -xzOf "$temporary/$asset" brb > "$staged" || fail 'could not extract brb'
    [ -s "$staged" ] || fail 'archive contains an empty binary'
    chmod 755 "$staged"
    mv -f "$staged" "$install_dir/brb"
    staged=
    printf 'Installed brokk-release-bot %s to %s/brb\n' "$version" "$install_dir"
    case ":${PATH:-}:" in
        *":$install_dir:"*) ;;
        *) printf 'Add %s to your PATH, then run brb --help.\n' "$install_dir" ;;
    esac
}
