...
func TestFixtureProcess(t *testing.T) {
	...
	if mode == "gh" {
		...
		if method == "POST" {
			if fixture.Create == "" || u.Path != "repos/o/r/issues" {
				os.Exit(93)
			}
			var raw struct {
				Proposals []struct {
			...
			readFixture(filepath.Join(fixture.State, "state.json"), &raw)
			title, body := "", ""
			for i, a := range args {
				if a == "-f" && i+1 < len(args) {
					if v, ok := strings.CutPrefix(args[i+1], "title="); ok {
						title = v
					}
			...
			for _, p := range raw.Proposals {
				if p.Status == "posting" && strings.Contains(body, p.RequestID) {
					intent = true
				}
			}
			if !intent {
			...
			issue := map[string]any{"number": 42, "title": title, "body": body, "state": "open", "html_url": "https://github.com/o/r/issues/42"}
			writeFixture(filepath.Join(root, "created.json"), issue)
			if fixture.Create == "lost" {
				os.Exit(1)
			}
		...
		if !fixture.Hidden && u.Path == "repos/o/r/issues" {
			if b, err := os.ReadFile(filepath.Join(root, "created.json")); err == nil {
				fmt.Printf("[%s]", b)
				os.Exit(0)
			}
		}
...
func writeFixture(path string, value any) {
