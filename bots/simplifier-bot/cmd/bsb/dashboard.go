...
func newDashboard(cfg bot.Config) *dashboard {
	...
	return &dashboard{cfg: cfg, started: now, phaseStarted: now, lastActivity: now,
		progress: bot.Progress{Phase: "starting", Task: "Loading saved proposals"}}
}
...
func (d *dashboard) render(width, height int, now time.Time, color bool) string {
	...
	add("SIMPLIFIER BOT  "+d.cfg.GitHubRepo(), "1;33")
	if d.detail && len(p.Items) > 0 {
		add(fmt.Sprintf("PROPOSAL DETAIL · %d/%d", d.selected+1, len(p.Items)), "2")
	...
	c := p.Counts
	add(fmt.Sprintf("Saved: %d proposed · %d filed", c.Found, c.Filed), "1")
	if height >= 18 {
		add(fmt.Sprintf("%d pending · %d dry run", c.Pending, c.DryRun), "2")
	}
	run := fmt.Sprintf("Run: %d scans · %d agents · %d tools", d.runs, d.sessions, d.tools)
	if width >= 65 {
		run = fmt.Sprintf("Run: %d scans · %d agents · %d tools · %d errors", d.runs, d.sessions, d.tools, d.failures)
	}
	add(run, "2")
	if height >= 18 {
		add("", "")
...
func (d *dashboard) findingRows(limit int) []string {
	...
	if len(findings) == 0 {
		if d.progress.Phase == "waiting" || d.progress.Phase == "complete" {
			return []string{"No proposals in saved history."}
		}
		return []string{"No proposals yet."}
	}
...
func (d *dashboard) summary(err error) string {
	...
	c := d.progress.Counts
	text := fmt.Sprintf("bsb %s · %s · saved: %d proposed, %d filed\n", cleanText(d.cfg.GitHubRepo()), status, c.Found, c.Filed)
