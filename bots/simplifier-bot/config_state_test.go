...
func TestReadConfigDefaultsAndPaths(t *testing.T) {
	...
	if cfg.Remote != filepath.Join(root, "mirror") || cfg.Directory != filepath.Join(root, "checkout") || cfg.StateDirectory != filepath.Join(root, "state") || cfg.GitHubRepo() != "o/r" || cfg.Timeout != DefaultConfig().Timeout || cfg.MaxProposals != DefaultConfig().MaxProposals {
...
func TestValidateConfig(t *testing.T) {
	for _, tc := range []struct {
		...
		{"zero limit", func(c *Config) { c.MaxProposals = 0 }},
		{"excess limit", func(c *Config) { c.MaxProposals = 20 + 1 }},
	} {
