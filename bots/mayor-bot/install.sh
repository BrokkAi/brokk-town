...
fail() {
    printf 'brokk-mayor-bot installer: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    if [ "$version" = latest ]; then
        ...
        registry=$(download "https://registry.npmjs.org/@brokkai/mayor-bot/latest") || fail 'could not resolve bot version'
        version=$(printf '%s' "$registry" | python3 -c 'import json,sys; print("v"+json.load(sys.stdin)["version"])') || fail 'invalid bot version'
    fi
    case "$version" in *-mayor-bot) ;; *) version=$version-mayor-bot ;; esac
    printf '%s\n' "$version" | awk '
    ...
    [ ! -d "$install_dir/bmb" ] || fail 'installation target is a directory'
    temporary=$(mktemp -d)
    staged=
    trap 'rm -rf "$temporary"; if [ -n "$staged" ]; then rm -f "$staged"; fi' 0
    trap 'exit 1' HUP INT TERM

    asset=brokk-mayor-bot-$version-$os-$arch.tar.gz
    printf 'Downloading brokk-mayor-bot %s for %s/%s...\n' "$version" "$os" "$arch"
    ...
    mkdir -p "$install_dir"
    staged=$(mktemp "$install_dir/.brokk-mayor-bot.XXXXXX")
    # Extract only the binary to a fresh file; never follow archive paths or links.
    tar -xzOf "$temporary/$asset" bmb > "$staged" || fail 'could not extract bmb'
    [ -s "$staged" ] || fail 'archive contains an empty binary'
    chmod 755 "$staged"
    mv -f "$staged" "$install_dir/bmb"
    staged=
    printf 'Installed brokk-mayor-bot %s to %s/bmb\n' "$version" "$install_dir"
    case ":${PATH:-}:" in
        *":$install_dir:"*) ;;
        *) printf 'Add %s to your PATH, then run bmb --help.\n' "$install_dir" ;;
    esac
}
