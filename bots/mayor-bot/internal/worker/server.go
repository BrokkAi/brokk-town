...
type Request struct {
	...
	// Arrival is the town's description of the item a judge run decides.
	Arrival json.RawMessage `json:"arrival,omitempty"`
	// Since and Until bound the window a bulletin run summarizes.
...
type Result struct {
	...
	Bulletin *BulletinResult `json:"bulletin,omitempty"`
}

// JudgmentResult is the Mayor's decision on the arrival a judge run named.
type JudgmentResult struct {
	Decision string `json:"decision"`
	Reason   string `json:"reason"`
}

// BulletinResult is the Mayor's bulletin for the window a bulletin run named,
// with the pull requests it covered so the town can advance its cursor.
...
func (s *server) runs(w http.ResponseWriter, r *http.Request) {
	...
	if result.Judgment != nil || result.Bulletin != nil {
