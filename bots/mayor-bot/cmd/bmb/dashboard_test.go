...
func dashboardFixture() *dashboard {
	...
	cfg.GitHub.Repo = "BrokkAi/mayor-bot"
	cfg.Branch = "main"
	cfg.Agent.Model = "review-model"
	cfg.StateDirectory = "/tmp/mayor-bot-state"
	d := newDashboard(cfg)
	d.update(bot.Progress{Phase: "starting"})
	d.update(bot.Progress{Phase: "writing", Task: "Summarizing 4 merged pull request(s) for users",
		Counts: bot.BulletinCounts{Bulletins: 3, Items: 7, Pulls: 9}, Items: []bot.ItemProgress{
			{ID: "a", Title: "Faster startup", Status: "Sep 21", URL: "https://github.com/BrokkAi/mayor-bot/pull/42", Body: strings.Repeat("FEATURE · Startup is faster.\n", 20)},
			{ID: "b", Title: "Quieter logs", Status: "Sep 22"},
			{ID: "c", Title: "Wide text 日本語 👩‍💻 é bulletin", Status: "Sep 23"},
		}})
	d.runs, d.sessions, d.tools = 2, 4, 23
	d.tool = "git log"
	for _, text := range []string{"Reading pull requests", "Summarizing", "Checking linked issues"} {
...
func executeWithRun(ctx context.Context, args []string, log *slog.Logger, run runFunc) error {
	return executeWith(ctx, args, log, commands{watch: run}, os.Stdout)
}
...
func TestDashboardFitsPaneAndUnicode(t *testing.T) {
	...
	for _, want := range []string{"BrokkAi/mayor-bot", "main", "WRITING", "3 bulletins", "7 items", "9 pull requests", "2 bulletins", "4 agents", "23 tools", "git log"} {
...
func TestDashboardItemDetailsRemainSelected(t *testing.T) {
	...
	if !strings.Contains(text, "SEP 21") || !strings.Contains(text, "https://github.com/") {
...
func TestDashboardSummaryKeepsLinksAndNewItems(t *testing.T) {
	...
	if !strings.Contains(summary, "pull/42") || !strings.Contains(summary, "3 bulletins, 7 items") {
		t.Fatalf("summary: %s", summary)
	}
	d.update(bot.Progress{Items: []bot.ItemProgress{{ID: "new", Title: "New bulletin", Status: "Sep 24", Body: "Summary\nStartup is faster"}}})
	if !strings.Contains(d.summary(nil), "Summary\nStartup is faster") {
		t.Fatal("new bulletin lost on exit")
	}
}
