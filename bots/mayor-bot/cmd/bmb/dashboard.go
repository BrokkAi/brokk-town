...
func newDashboard(cfg bot.Config) *dashboard {
	...
	return &dashboard{cfg: cfg, started: now, phaseStarted: now, lastActivity: now,
		progress: bot.Progress{Phase: "starting", Task: "Loading recorded bulletins"}}
}
...
func (d *dashboard) render(width, height int, now time.Time, color bool) string {
	...
	add("MAYOR BOT  "+d.cfg.GitHubRepo(), "1;34")
	if d.detail && len(p.Items) > 0 {
		add(fmt.Sprintf("BULLETIN DETAIL · %d/%d", d.selected+1, len(p.Items)), "2")
	...
	c := p.Counts
	add(fmt.Sprintf("Recorded: %d bulletins · %d items · %d pull requests", c.Bulletins, c.Items, c.Pulls), "1")
	if height >= 18 {
		add("Every "+elapsed(time.Duration(d.cfg.Poll))+" · never writes to GitHub", "2")
	}
	run := fmt.Sprintf("Run: %d bulletins · %d agents · %d tools", d.runs, d.sessions, d.tools)
	if width >= 65 {
		run = fmt.Sprintf("Run: %d bulletins · %d agents · %d tools · %d errors", d.runs, d.sessions, d.tools, d.failures)
	}
	add(run, "2")
	if height >= 18 {
		add("", "")
	}
	tabs := []string{"1 Overview", "2 Bulletins", "3 Activity"}
...
func (d *dashboard) findingRows(limit int) []string {
	...
	if len(findings) == 0 {
		if d.progress.Phase == "waiting" || d.progress.Phase == "complete" {
			return []string{"No bulletins recorded yet."}
		}
		return []string{"No bulletins yet."}
	}
...
func (d *dashboard) summary(err error) string {
	...
	c := d.progress.Counts
	text := fmt.Sprintf("bmb %s · %s · recorded: %d bulletins, %d items\n", cleanText(d.cfg.GitHubRepo()), status, c.Bulletins, c.Items)
