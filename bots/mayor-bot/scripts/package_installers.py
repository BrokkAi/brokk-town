...
def package(tag, assets, output, sha):
    ...
    packages = []
    base = {
        "version": npm_version, "license": "MIT",
        "repository": {"type": "git", "url": "git+https://github.com/BrokkAi/brokk-town.git"},
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for target in release.TARGETS:
            ...
            with tarfile.open(assets / name, "r:gz") as bundle:
                files = {member: bundle.extractfile(member).read() for member in ("bmb", "README.md", "BUILD.json", *licenses.LEGAL_FILES)}
            ...
            npm_pack(package_name, {"os": [system], "cpu": [arch], "description": f"Brokk Mayor Bot native binary for {system}/{arch}"},
                     {"bin/bmb": files["bmb"], **{name: files[name] for name in licenses.LEGAL_FILES}, "README.md": files["README.md"], "BUILD.json": files["BUILD.json"]})
        npm_pack(NPM_ROOT, {
            "description": "Brokk Mayor Bot: autonomous issue fixes through Agent Client Protocol",
            "bin": {"bmb": "bin/bmb.cjs"}, "engines": {"node": ">=18"},
            "os": ["linux", "darwin"], "cpu": ["x64", "arm64"], "optionalDependencies": dependencies,
        }, {"bin/bmb.cjs": (ROOT / "npm/bmb.cjs").read_bytes(), **{name: files[name] for name in licenses.LEGAL_FILES}, "README.md": files["README.md"]})
