...
def context(tag_arg=None, sha_arg=None):
    ...
    if not tag.endswith("-bug-bot"):
        raise ValueError("release tag must end in -bug-bot")
    ...
    if manifest_path.exists():
        ...
        entry = next(b for b in entries if b["project"] == "bug-bot")
...
def publish(directory, tag, sha, check_only=False):
    ...
    if record is None:
        record = api("releases", "POST", {
            "tag_name": tag, "target_commitish": sha, "name": f"bug-bot {tag}",
