...
func TestWorkerReusesProcessRejectsOverlapAndCancelsActiveRun(t *testing.T) {
	...
	go func() { done <- Serve(ctx, socket, lifecycleInfo(), run, nil) }()
...
func TestWorkerParentPipeChild(t *testing.T) {
	...
	err := Serve(context.Background(), socket, lifecycleInfo(), func(context.Context, Request, func(Progress)) (Result, error) { return Result{}, nil }, nil)
