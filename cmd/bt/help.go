...
var cliCommands = []commandInfo{
	...
	{name: "execution", short: "List or select execution targets", long: "List cached Mjolnir launch options. --refresh queues a background refresh. Use --repo and --target/--profile to save a selection, --local-execution to run directly here, or --inherit-execution --role BOT to restore inheritance. --runtime-session ID explicitly selects the target runtime from an initialized Mjolnir session. Managed PR review and repairs from verified review feedback require a selected runtime; other agent duties remain held when assigned to Mjolnir.", args: "[flags]", flags: []string{"json", "refresh", "repo", "role", "target", "profile", "runtime-session", "local-execution", "inherit-execution"}},
