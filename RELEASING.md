...
Run `make check smoke`, inspect `git diff`, and commit the complete change.
...
Python publication is not configured. The browser assets are embedded in `bt`;
there is no separate hosted website deployment.
...
The npm namespace contains `@brokkai/brokk-town` and four platform packages:
`@brokkai/brokk-town-linux-x64`, `@brokkai/brokk-town-linux-arm64`,
`@brokkai/brokk-town-darwin-x64`, and `@brokkai/brokk-town-darwin-arm64`.
The GitHub `packages-publish` environment must have a deployment tag policy
matching `v*` (configured with the repository setup). From a clean committed checkout, build and validate a complete
...
```sh
for package in brokk-town brokk-town-linux-x64 brokk-town-linux-arm64 brokk-town-darwin-x64 brokk-town-darwin-arm64; do
  npm trust github "@brokkai/$package" --repo BrokkAi/brokk-town \
...
Match the reference bot's npm access before the stable release: all five packages
must be public and grant the `brokkai:developers` team read/write access. Compare
`npm access list collaborators` and `npm owner ls` for each against
`@brokkai/review-bot`; preserve the organization's existing maintainer access.
...
and exercises `bt --help`. It does not publish anything. No Python package
is configured for brokk-town.
