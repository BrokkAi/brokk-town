...
The current candidate is **v0.3.0**. All deliverables use the exact same
...
```sh
export RELEASE_COMMIT="$(git rev-parse HEAD)"
export PREFLIGHT_TAG="v0.3.0-preflight.$(git rev-parse --short=8 "$RELEASE_COMMIT")"
...
gh workflow run publish-packages.yml --repo github.com/BrokkAi/brokk-town \
  --ref "$PREFLIGHT_TAG" -f tag=v0.3.0 -F publish=false
```

Check the returned run's exact SHA. The preflight ref is immutable and is not
the release version; the workflow input remains `v0.3.0` so its candidate and
...
broadening it. Do not create the final `v0.3.0` tag merely to test access and do
