  ...
  $("#houses").innerHTML = Object.entries(positions)
    ...
    .map(([role, [x, y]]) => {
      const w = t?.workers[role],
        ...
        status = role === "hall" ? `${pendingDecisions(t)} to decide` : worker.status,
        dot =
...
function renderInspection() {
  ...
  if (selectedHouse === "hall") {
    ...
    const autoMayorBlock = `<div class="inspector-actions"><button id="auto-mayor-toggle"${autoMayor ? "" : ' class="primary"'}>${autoMayor ? "Turn off Auto-Mayor" : "Turn on Auto-Mayor"}</button></div><p class="muted">${autoMayor ? "Auto-Mayor is on: the town's agent judges each arrival from the source text, any Simplifier Bot advice or Town review, and the code, and leaves its reason on the task. A judgment it cannot finish after three attempts waits here for you." : "Auto-Mayor has the town's agent judge arrivals for you, one at a time, with a reason on each decision. Anything already waiting here is judged as soon as it is turned on."}</p>`;
