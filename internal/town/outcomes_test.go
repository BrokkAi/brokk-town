...
func TestOutcomeLedgerCoversLifecycleAndDeduplicatesReconciliation(t *testing.T) {
	...
	Reconcile(&state, town, RepoSnapshot{Branch: "master", Head: merged.MergeCommit, Pulls: []Pull{external}, Releases: []RemoteRelease{release}, Released: map[string]bool{}}, start.Add(7*time.Minute))
	external.Head.SHA = strings.Repeat("f", 40)
	Reconcile(&state, town, RepoSnapshot{Branch: "master", Head: merged.MergeCommit, Pulls: []Pull{external}, Releases: []RemoteRelease{release}, Released: map[string]bool{}}, start.Add(8*time.Minute))

	report, err := BuildOutcomeReport(state, town.ID, start.Add(-time.Minute), start.Add(time.Hour), start.Add(time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if report.Summary.FindingsFiled != 1 || report.Summary.PRsSubmitted != 1 || report.Summary.RepairRounds != 1 || report.Summary.MergesConfirmed != 1 || report.Summary.ReleasesVerified != 1 || report.Summary.BlockedAbandoned != 1 || report.Summary.FalsePositives != 1 || report.Summary.Useful != 0 {
	...
	for _, record := range report.Records {
		...
		if record.ElapsedMS != nil || record.Usage != nil || record.CostUSD != nil {
