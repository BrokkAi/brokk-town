...
func TestAutoUpdateAppliesNewBotVersionsWithoutADecision(t *testing.T) {
	...
	supervisor.now = func() time.Time { return now }
	on := true
	if err := supervisor.SettingsForRoleAndPolicy(x.ID, "", AgentSettings{}, nil, nil, nil, &on); err != nil {
		t.Fatal(err)
	}
...
func TestExplicitPinWithdrawsAnOfferItCaughtUpWith(t *testing.T) {
	...
	supervisor.now = func() time.Time { return now }
	pin := "9.0.0"
	if err := supervisor.SettingsForRoleAndPolicy(x.ID, Feature, AgentSettings{}, nil, nil, &pin, nil); err != nil {
		t.Fatal(err)
	}
	if upgradeTask(t, store, Feature) != nil {
